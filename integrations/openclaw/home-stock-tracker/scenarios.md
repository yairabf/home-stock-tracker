...
| Household inventory view | The user asks what the household probably has. | Call `list_inventory({})` once; label `current` as estimated stock and keep `uncertain` separate. | Return only the materialized household view without treating omitted products as known absent. |
| Expiration status view | The user asks what might expire soon. | Call `list_expiration_status({})` once and report every returned batch status with its expiry source. | Do not claim a batch is still on hand, change stock, or add a grocery item from this read. |
| Combined grocery list and suggestions | The user says, "Show me the list." | Call `grocery_list({})` and `get_low_stock_predictions({})`; present committed items and suggestions as separate labeled groups. | No suggestion is represented as committed and no mutation occurs. |
| Confirmed suggestion addition | After seeing suggestions, the user explicitly asks to add one named suggestion. | Use the returned product name in the normal `grocery_add` proposal workflow only after confirmation. | Only the confirmed suggestion enters the grocery workflow; other suggestions remain read-only. |
| Recently purchased product list | The user reports several resolved products purchased yesterday with measurements attached to specific items. | Call `record_purchases` once with unique trusted product IDs, preserving item order, measurements, and the explicit timestamp. | All ordered purchase receipts are returned, or none of the batch is recorded. |
| Unresolved batch purchase | One product in a recently purchased list cannot be resolved exactly. | Ask the user to resolve that product before sending the batch; do not record the already resolved prefix. | No purchase in the list is recorded. |
| Batch purchase transport uncertainty | The atomic `record_purchases` call ends without a reliable result. | Stop and report that the whole batch outcome is uncertain; do not retry or continue with individual writes. | No success claim is made and duplicate purchases are avoided. |
| Explicit stock set | The user reports an exact current quantity and unit for one resolved product. | Call `update_inventory` with `operation: "set"` and only the supplied measurement. | The explicit stock fact and resulting materialized projection are returned. |
