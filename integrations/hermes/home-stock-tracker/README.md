...
For an expiration-status smoke check, ask "What might expire soon?" Hermes should
call `list_expiration_status` once and report its returned batch status and expiry
source without claiming that a batch is still on hand or changing stock.
