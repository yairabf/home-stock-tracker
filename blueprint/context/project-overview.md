...
- **MCP tools** (service-exposed or via a thin adapter) - `grocery_add`, `grocery_set_quantity`, `grocery_update`, `grocery_remove`, `grocery_list`, `record_purchase`, `record_stock_signal`, `get_inventory`, `get_product`, `get_low_stock_predictions`. Hermes learns tool usage via a dedicated skill; business rules stay in the service, not in Hermes.
