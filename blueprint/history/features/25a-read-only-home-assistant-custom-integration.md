...
- A repository-owned Home Assistant custom integration under
  `integrations/home-assistant/custom_components/home_stock_tracker/`.
...
- Any Home Assistant service, automation, webhook, voice intent, or direct REST
  ...
  data. That is feature 25b and must retain the service's explicit confirmation,
  optimistic-concurrency, and no-uncertain-retry rules.
- Push updates, webhooks, MQTT, Home Assistant discovery, cloud publishing, HACS
  publication, or changes to NestJS REST/MCP contracts.
...
- [x] **Step 4 - Document installation and prove the integration boundary** - add a concise local-install and configuration guide, then run the custom-component tests alongside the existing service gates. *Done when:* the guide identifies the custom-components path, required base URL and service token, three entity meanings, polling behavior, unavailable behavior, and read-only limitation; fixture-driven tests cover setup, reauth, success, empty data, malformed data, and service failures; `npm run test`, `npm run test:e2e`, `npm run build`, `git diff --check`, the Home Assistant test command, and `graphify update .` pass.

## Files / areas

- `integrations/home-assistant/custom_components/home_stock_tracker/` - manifest,
  config flow, coordinator, API client, sensors, translations, and constants.
- `integrations/home-assistant/tests/` and its pinned test requirements - focused
  Home Assistant fixtures and integration behavior.
- `docs/agent-integrations.md` or a new focused Home Assistant guide - local
  install, configuration, entity, and safety documentation.
...
- These contracts are load-bearing for 25b: mutation services must reuse the
  same authenticated client and must not bypass the upstream REST contracts.
...
- Keep existing NestJS tests unchanged unless documentation or a service contract
  proves inaccurate. The existing Jest gate remains required: `npm run test` and
  `npm run test:e2e`; final fallback gates are `npm run build` and
  `git diff --check`.
...
- This repository is primarily NestJS, but the Home Assistant integration is
  Python. Keep it isolated under `integrations/home-assistant`; do not add Python
  runtime dependencies to the NestJS application.
...
- Do not start implementation until this spec is reviewed. After code changes,
  run `graphify update .`.
